...
Arguments:
    new_version  Version string for the new release (e.g. "0.2.0").
...
# Stable release tag pattern — tags in this repo have no 'v' prefix (e.g. 0.1.1)
...
def latest_stable_tag():
    ...
    for tag in tags:
        if STABLE_TAG_RE.match(tag.strip()):
            return tag.strip()
    sys.exit("Error: no stable release tag found (expected X.Y.Z)")


def next_version(tag):
    """Auto-increment the patch component of an X.Y.Z tag."""
...
def main():
    ...
    version = args[0].lstrip("v") if args else next_version(tag)
    if not SEMVER_RE.match(version):
        sys.exit(f"Error: {args[0]!r} is not a valid version; expected X.Y.Z")
    ...
    # Prepend directly — CHANGELOG has no top-level heading to skip past
